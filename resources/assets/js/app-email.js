  ...
  (function () {
    ...
    if (selectors.emailCompose) {
      selectors.emailCompose.addEventListener('hidden.bs.modal', () => {
        const editor = document.querySelector('.email-editor .ql-editor');
        if (editor) editor.innerHTML = '';
        const contacts = document.getElementById('emailContacts');
        if (contacts) contacts.value = '';
        initSelect2();
      });
    }
    ...
    if (selectors.emailFilterFolderLists) {
      selectors.emailFilterFolderLists.forEach(folder => {
        folder.addEventListener('click', () => {
          if (selectors.emailViewContainer) {
