...
const securityDashboard = {
    ...
    async handleBlockIp(event) {
        ...
        try {
            const response = await fetch('/dashboard/security/blocked-ips', {
                method: 'POST',
                headers: {
    ...
    async handleResolveLog(event) {
        ...
        try {
            ...
            if (notes) {
                const response = await fetch(`/dashboard/security/logs/${logId}/resolve`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    ...
    initCharts() {
        ...
        // Event Distribution Chart (donut)
        const distributionEl = document.querySelector('#eventDistributionChart');
        if (distributionEl) {
            const distributionConfig = {
                series: JSON.parse(distributionEl.dataset.events || '[]'),
                chart: { type: 'donut', height: 350 },
                labels: JSON.parse(distributionEl.dataset.labels || '[]'),
                legend: { position: 'bottom' },
                dataLabels: { enabled: true }
            };
            const distributionChart = new ApexCharts(distributionEl, distributionConfig);
            distributionChart.render();
        }

        // Response Time Trend Chart
        const responseChartEl = document.querySelector('#responseTimeChart');
        if (responseChartEl) {
            const responseConfig = {
                series: [{ name: 'Avg Resolution (min)', data: JSON.parse(responseChartEl.dataset.times || '[]') }],
                chart: { height: 350, type: 'line', toolbar: { show: false } },
                stroke: { curve: 'smooth', width: 3 },
                xaxis: { type: 'datetime', categories: JSON.parse(responseChartEl.dataset.dates || '[]') },
                yaxis: { labels: { formatter: val => Math.round(val) } },
                tooltip: { x: { format: 'dd MMM yyyy' } }
            };
            const responseChart = new ApexCharts(responseChartEl, responseConfig);
