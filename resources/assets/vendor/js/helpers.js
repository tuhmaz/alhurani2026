...
const Helpers = {
  ...
  _bindMenuMouseEvents() {
    ...
    if (!this._menuMouseEnter) {
      ...
      // Make touchstart passive to avoid blocking scrolling and reduce reflow pressure
      layoutMenu.addEventListener('touchstart', this._menuMouseEnter, { passive: true })
    }
    ...
    if (!this._windowTouchStart) {
      ...
      // Capture + passive to minimize main-thread work during touch
  ...
  update() {
    // Compute whether we need inline style updates and read layout first
    const hasNavbar = this.getLayoutNavbar()
    const hasFooter = this.getLayoutFooter()
    const needNavbarPad = hasNavbar && ((!this.isSmallScreen() && this.isLayoutNavbarFull() && this.isFixed()) || this.isNavbarFixed())
    const needFooterPad = hasFooter && this.isFooterFixed()

    let navbarH = 0, footerH = 0
    if (needNavbarPad) navbarH = this._getNavbarHeight()
    if (needFooterPad) footerH = this._getFooterHeight()

    // Defer writes to next frame to avoid sync layout thrash
    requestAnimationFrame(() => {
      if (needNavbarPad || needFooterPad) {
        this._updateInlineStyle(navbarH, footerH)
      }
