@php
  ...
  $defaultLogo = 'assets/img/logo/logo.webp';
    ...
    <div class="navbar navbar-expand-lg landing-navbar px-3 px-md-8">
      ...
      <a href="{{ url('/') }}" class="navbar-brand app-brand demo py-0 py-lg-2 me-0 me-lg-4">
        <div class="app-brand-link d-flex align-items-center gap-2">
          @php
            $frontLogoUrl = isset($siteSettings['site_logo']) ? asset('storage/' . $siteSettings['site_logo']) : asset($defaultLogo);
            $frontLogoPath = ltrim(parse_url($frontLogoUrl, PHP_URL_PATH) ?? '', '/');
            $front32 = $frontLogoPath ? route('img.fit', ['size' => '32x32', 'path' => $frontLogoPath]) : $frontLogoUrl;
            $front64 = $frontLogoPath ? route('img.fit', ['size' => '64x64', 'path' => $frontLogoPath]) : $frontLogoUrl;
          @endphp
          <img
            src="{{ $front32 }}"
            srcset="{{ $front32 }} 1x, {{ $front64 }} 2x"
            sizes="32px"
            alt="{{ $siteSettings['site_name'] }}" class="app-brand-logo" width="32" height="32"
            decoding="async" fetchpriority="low">
          <span class="app-brand-text menu-text fw-bold d-none d-md-inline-block"
            style="color: #000;">{{ $siteSettings['site_name'] }}</span>
        </div>
      </a>
        ...
        <div class="d-flex align-items-center gap-2">
          ...
          <div class="dropdown">
            <form method="POST" action="{{ route('setDatabase') }}" id="databaseForm" class="mb-0">
              ...
              <button type="button"
                class="btn btn-outline-warning btn-icon rounded-pill dropdown-toggle hide-arrow p-1 p-sm-2"
        ...
        "
              ...
              <ul class="dropdown-menu dropdown-menu-end mt-1">
                  ...
                  <a class="dropdown-item" href="{{ route('dashboard.users.show', Auth::user()->id ?? '') }}">
                    <div class="d-flex align-items-center">
                      <div class="flex-shrink-0 me-3">
                        <div class="avatar avatar-online">
                          <img
                            src="{{ Auth::check() && Auth::user()->profile_photo_path ? (Str::startsWith(Auth::user()->profile_photo_path, ['http://','https://','//']) ? Auth::user()->profile_photo_path : asset('storage/' . Auth::user()->profile_photo_path)) : asset($defaultAvatar) }}"
                ...
                <li>
                  <a class="dropdown-item d-flex align-items-center" href="{{ route('dashboard.index') }}">
                    <i class="nav-icon ti tabler-dashboard me-2 ti-sm"></i>
                ...
                <li>
                  <form method="POST" action="{{ route('logout') }}" class="mb-0">
                    ...
                    <button type="submit" class="dropdown-item d-flex align-items-center">
                      <i class="nav-icon ti tabler-logout me-2 ti-sm"></i>
          ...
          @else
            <a href="{{ route('login') }}" class="btn btn-primary btn-sm d-flex align-items-center"
              aria-label="{{ __('Login/Register') }}" title="{{ __('Login/Register') }}">
              <i class="nav-icon ti tabler-login me-1 ti-sm" aria-hidden="true"></i>
              <span class="visually-hidden">{{ __('Login/Register') }}</span>
