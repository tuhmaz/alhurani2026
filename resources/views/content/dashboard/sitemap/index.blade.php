            ...
            <div class="card-header border-bottom">
                <div class="d-flex align-items-center justify-content-between">
                    <h5 class="card-title mb-0">
                        <i class="sitemap-icon ti tabler-sitemap me-2"></i>{{ __('Sitemap Management') }}
                    </h5>
                </div>
            </div>
            ...
            <div class="card-body">
                @if(session('success'))
                    <div class="alert alert-success alert-dismissible fade show" role="alert">
                        <i class="sitemap-icon ti tabler-check-circle me-2"></i>
                        {{ session('success') }}
                ...
                <div class="alert alert-info bg-info bg-opacity-10 border-0">
                    <div class="d-flex">
                        <i class="sitemap-icon ti tabler-info-circle fs-4 me-2"></i>
                        <div>
                            ...
                            <div class="card bg-label-primary bg-opacity-10 shadow-none">
                                <div class="card-body">
                                    <div class="d-flex align-items-center mb-3">
                                        <i class="sitemap-icon ti tabler-database fs-3 me-2"></i>
                                        <h5 class="mb-0">{{ __('Select Database for Sitemap Generation') }}</h5>
                                    </div>
                                    <p class="text-muted">{{ __('Choose the database you want to generate a sitemap for. This will create XML sitemaps for articles, posts, and static pages.') }}</p>
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-4">
                                            <button type="submit" class="btn btn-primary btn-lg w-100 h-100">
                                                <i class="sitemap-icon ti tabler-refresh me-2"></i>{{ __('Generate Sitemap') }}
                                            </button>
                                        </div>
                                    </div>
                                </div>
                ...
                <h5 class="mb-3">
                    <i class="sitemap-icon ti tabler-files me-2"></i>{{ __('Available Sitemap Links') }}
                </h5>
                        ...
                        <div class="accordion-item">
                            <h2 class="accordion-header">
                                <button class="accordion-button {{ $loop->first ? '' : 'collapsed' }}" type="button"
                                    ...
                                    <i class="sitemap-icon ti tabler-world me-2"></i>{{ __('Sitemap for') }} {{ $dbNames[$db] }}
                                </button>
                            </h2>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-hover mb-0">
                                            ...
                                            <tbody>
                                                    ...
                                                    <td>
                                                        <a href="{{ asset('storage/sitemaps/sitemap_articles_' . $db . '.xml') }}"
                                                           class="d-flex align-items-center text-body" target="_blank">
                                                            <i class="sitemap-icon ti tabler-article me-2"></i>
                                                            <span>{{ __('Articles Sitemap') }}</span>
                                                            <i class="sitemap-icon ti tabler-external-link ms-2 text-muted"></i>
                                                        </a>
                                                    </td>
                                                    <td>
                                                        @if(isset($sitemapData['articles']) && $sitemapData['articles']['exists'])
                                                            <span class="text-muted">
                                                                <i class="sitemap-icon ti tabler-clock me-1"></i>
                                                    ...
                                                    <td class="text-end">
                                                        <form action="{{ route('dashboard.sitemap.delete', ['type' => 'articles', 'database' => $db]) }}"
                                                            ...
                                                            <button type="submit" class="btn btn-sm btn-danger"
                                                                ...
                                                                <i class="sitemap-icon ti tabler-trash me-1"></i>{{ __('Delete') }}
                                                            </button>
                                                        </form>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        <a href="{{ asset('storage/sitemaps/sitemap_post_' . $db . '.xml') }}"
                                                           class="d-flex align-items-center text-body" target="_blank">
                                                            <i class="sitemap-icon ti tabler-file-text me-2"></i>
                                                            <span>{{ __('Post Sitemap') }}</span>
                                                            <i class="sitemap-icon ti tabler-external-link ms-2 text-muted"></i>
                                                        </a>
                                                    </td>
                                                    <td>
                                                        @if(isset($sitemapData['post']) && $sitemapData['post']['exists'])
                                                            <span class="text-muted">
                                                                <i class="sitemap-icon ti tabler-clock me-1"></i>
                                                                {{ \Carbon\Carbon::createFromTimestamp($sitemapData['post']['last_modified'])->diffForHumans() }}
                                                            </span>
                                                        @else
                                                            <span class="badge bg-label-warning">{{ __('Not Generated') }}</span>
                                                        @endif
                                                    ...
                                                    <td class="text-end">
                                                        <form action="{{ route('dashboard.sitemap.delete', ['type' => 'post', 'database' => $db]) }}"
                                                              method="POST" class="d-inline-block">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="btn btn-sm btn-danger"
                                                ...
                                                <tr>
                                                    <td>
                                                        <a href="{{ asset('storage/sitemaps/sitemap_static_' . $db . '.xml') }}"
                                                           class="d-flex align-items-center text-body" target="_blank">
                                                            <i class="sitemap-icon ti tabler-file me-2"></i>
