    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap">
      <div>
        ...
        <div class="article-meta">
          <span><i class="article-icon ti tabler-calendar me-1"></i>{{ $article->created_at->format('Y-m-d H:i') }}</span>
          <span><i class="article-icon ti tabler-bookmark me-1"></i>{{ $article->subject->subject_name ?? __('Not Set') }}</span>
          <span><i class="article-icon ti tabler-school me-1"></i>{{ $article->schoolClass->grade_name ?? __('Not Set') }}</span>
        </div>
      </div>
      <div class="action-buttons">
        @if(!$article->status)
        <button type="button" class="action-btn btn btn-success publish-btn" data-id="{{ $article->id }}" data-action="publish">
          <i class="article-icon ti tabler-check"></i>{{ __('Publish') }}
        </button>
        @else
        <button type="button" class="action-btn btn btn-warning unpublish-btn" data-id="{{ $article->id }}" data-action="unpublish">
          <i class="article-icon ti tabler-x"></i>{{ __('Unpublish') }}
        </button>
        @endif

        <a href="{{ route('dashboard.articles.edit', ['article' => $article->id, 'country' => $country]) }}"
           class="action-btn btn btn-primary">
          <i class="article-icon ti tabler-edit"></i>{{ __('Edit') }}
        </a>

        <button type="button" class="action-btn btn btn-label-secondary print-article">
          <i class="article-icon ti tabler-printer"></i>{{ __('Print') }}
        </button>

        <button type="button" class="action-btn btn btn-label-primary share-article">
          <i class="article-icon ti tabler-share"></i>{{ __('Share') }}
        </button>
        ...
        <form action="{{ route('dashboard.articles.destroy', ['article' => $article->id, 'country' => $country]) }}"
          ...
          <button type="submit" class="action-btn btn btn-danger">
            <i class="article-icon ti tabler-trash"></i>{{ __('Delete') }}
          </button>
        </form>
      </div>
    </div>
        ...
        <div class="attachments-section">
          ...
          <div class="attachment-card">
            <div class="attachment-icon">
              <i class="article-icon ti tabler-file"></i>
            </div>
            <div class="attachment-info">
              <div class="attachment-name">{{ $file->file_name }}</div>
              <div class="attachment-meta">
                <span class="badge bg-label-{{ $file->file_category == 'plans' ? 'primary' : (
                  $file->file_category == 'papers' ? 'info' : (
                  $file->file_category == 'tests' ? 'warning' : (
                  $file->file_category == 'books' ? 'success' : 'secondary'))
                ) }}">
          ...
          <div class="card-body">
            ...
            <div class="keywords-container">
              ...
              <span class="badge bg-label-primary rounded-pill mb-1">
                <i class="article-icon ti tabler-tag me-1"></i>
                {{ $keyword->keyword }}
              </span>
              @endforeach
            </div>
