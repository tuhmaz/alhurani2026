...
$(function () {
  ...
  const dt_articles = $('#articlesTable').DataTable({
      ...
      },
      {
        ...
        render: function (data, type, row) {
          return '<div class="d-flex gap-2">' +
            '<a href="' + row.show_url + '" class="btn btn-sm btn-info"><i class="page-icon ti tabler-eye"></i></a>' +
            '<a href="' + row.edit_url + '" class="btn btn-sm btn-primary"><i class="page-icon ti tabler-pencil"></i></a>' +
            '<form action="' + row.delete_url + '" method="POST" class="delete-form d-inline">' +
              '<input type="hidden" name="_token" value="' + $('meta[name="csrf-token"]').attr('content') + '">' +
              '<input type="hidden" name="_method" value="DELETE">' +
              '<button type="submit" class="btn btn-sm btn-danger"><i class="page-icon ti tabler-trash"></i></button>' +
            '</form>' +
          '</div>';
        }
      }
    ...
    language: {
      ...
      paginate: {
        previous: '<i class="page-icon ti tabler-chevron-left"></i>',
        next: '<i class="page-icon ti tabler-chevron-right"></i>'
      }
    },
