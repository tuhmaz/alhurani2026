...
class CalendarController extends Controller
{
    ...
    public function getEvents(Request $request)
    {
        try {
            ...
            // Validate database against allowed list
            $availableDatabases = collect(config('database.connections'))
                ->keys()
                ->filter(function ($name) {
                    return in_array($name, ['mysql', 'jo', 'sa', 'eg', 'ps']);
                })
                ->toArray();
            if (!in_array($database, $availableDatabases, true)) {
            ...
            $events = $query->orderBy('event_date', 'asc')->get()->map(function ($event) use ($database) {
            ...
            // Return empty list on any error to avoid 500s in calendar loading
            Log::warning('Calendar Events Error (soft-return empty):', [
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Validate and use requested database
            $database = $validated['eventDatabase'];
            $allowedDatabases = collect(config('database.connections'))
                ->keys()
                ->filter(function ($name) { return in_array($name, ['mysql', 'jo', 'sa', 'eg', 'ps']); })
                ->toArray();
            ...
            Log::error('Event Creation QueryException:', [
                'message' => $e->getMessage(),
                'code' => $e->getCode(),
                'database' => $database ?? 'unknown',
                'data' => $request->all()
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'Database error while creating event'
            ], 422);
        } catch (Throwable $e) {
