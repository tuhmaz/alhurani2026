...
class ImageUploadController extends Controller
{
        ...
        try {
            ...
            try {
                ...
                if ($request->input('create_responsive', false)) {
                    ...
                    // قراءة أبعاد الصورة
                    $image = Image::read(Storage::disk('public')->get($mainImagePath));
                    ...
                    // قراءة أبعاد الصورة
                    $image = Image::read(Storage::disk('public')->get($newPath));
    ...
    public function uploadFile(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            // الحفاظ على الاسم الحقيقي للملف مع دعم العربية وإزالة الرموز غير الآمنة
            $originalName = $file->getClientOriginalName();
            $extension = $file->getClientOriginalExtension();
            $base = pathinfo($originalName, PATHINFO_FILENAME);

            // السماح بالأحرف العربية واللاتينية والأرقام والمسافات والشرطات والشرطة السفلية والأقواس
            ...
            // بناء اسم الملف والمحافظة على الامتداد الأصلي
            $filename = $base . '.' . $extension;

            // إذا كان الاسم موجوداً مسبقاً، نضيف رقم متزايد في آخر الاسم
            $counter = 2;
            ...
            // تخزين الملف على قرص public لضمان إمكانية الوصول عبر /storage
            Storage::disk('public')->putFileAs('files', $file, $filename);

            // إرجاع رابط الويب للملف المرفوع
            return response()->json(['url' => Storage::url('files/' . $filename), 'name' => $filename]);
        }
