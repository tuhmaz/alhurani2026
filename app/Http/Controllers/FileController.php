  ...
  public function download(Request $request, $id)
  {
    try {
      ...
      if (\Illuminate\Support\Str::startsWith($relativePath, 'storage/')) {
        // عندما يكون المسار محفوظًا مسبقًا داخل مجلد public/storage
        $filePath = public_path($relativePath);
      } else {
        // عندما يكون المسار بالنسبة إلى تخزين Laravel العام
  ...
  public function update(Request $request, $id)
  {
    try {
      ...
      $request->validate([
        ...
        'file_category' => 'required|in:plans,papers,tests,books,records',
