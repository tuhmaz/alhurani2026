...
class MessageController extends Controller
{
    ...
    public function index(Request $request)
    {
        $users = User::all();
        $user_id = Auth::id();
        $totalMessages = Message::where(function ($q) use ($user_id) {
        ...
        $unreadMessagesCount = Message::whereIn('conversation_id', Conversation::where('user1_id', $user_id)->orWhere('user2_id', $user_id)->pluck('id'))
            ...
            ->where('read', false)
            ->count();
        $sentMessagesCount = Message::where('sender_id', $user_id)->where('is_chat', false)->count();
        ...
        if ($request->ajax() || $request->boolean('partial')) {
            return view('content.dashboard.messages.partials.list-inbox', compact('messages', 'unreadMessagesCount', 'sentMessagesCount'));
        }
    ...
    {
        // Recipient is required to bind the draft to a conversation due to non-nullable conversation_id
        $validated = $request->validate([
            'recipient' => 'required|exists:users,id',
        ...
        $sender_id = Auth::id();
        $recipient_id = (int) $request->recipient;

        // Ensure a conversation exists between sender and recipient
        $conversation = Conversation::firstOrCreate(
            ['user1_id' => min($sender_id, $recipient_id), 'user2_id' => max($sender_id, $recipient_id)]
        );
        ...
        return redirect()->route('dashboard.messages.drafts')->with('success', __('Draft saved successfully!'));
    }

    public function sent(Request $request)
    {
        $user_id = Auth::id();
        $messages = Message::where('sender_id', $user_id)->where('is_chat', false)->latest()->get();
        $sentMessages = Message::where('sender_id', $user_id)->where('is_chat', false)->latest()->get();
        $unreadMessagesCount = Message::whereIn('conversation_id', Conversation::where('user1_id', $user_id)->orWhere('user2_id', $user_id)->pluck('id'))
            ->where('is_chat', false)
            ->where('sender_id', '!=', $user_id)
            ->where('read', false)
        ...
        $sentMessagesCount = Message::where('sender_id', $user_id)->where('is_chat', false)->count();
        if ($request->ajax() || $request->boolean('partial')) {
            return view('content.dashboard.messages.partials.list-sent', compact('sentMessages', 'unreadMessagesCount', 'sentMessagesCount'));
        }
    ...
    {
        return view('content.dashboard.messages.important', ['messages' => Message::where('is_important', true)->where('is_chat', false)->get()]);
    }
    ...
    public function drafts(Request $request)
    {
        ...
        if ($request->ajax() || $request->boolean('partial')) {
            return view('content.dashboard.messages.partials.list-drafts', compact('draftMessages'));
        }
    ...
    public function trash(Request $request)
{
    ...
    $unreadMessages = Message::whereIn('conversation_id', $conversations)
        ...
        ->get();
    if ($request->ajax() || $request->boolean('partial')) {
        return view('content.dashboard.messages.partials.list-trash', compact('sentMessages', 'unreadMessages'));
    }
